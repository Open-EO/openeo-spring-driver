...
public interface ProcessGraphsApi {
	 ...
	 * Deletes the data related to this user-defined process, including its process
	 * graph. Does NOT delete jobs or services that reference this user-defined
	 * process.
	 *
	 * @param processGraphId Unique identifier for a user-defined process.
	 *                       (required)
	 * @return The user-defined process has been successfully deleted (status code
	 *         204) or The request can&#39;t be fulfilled due to an error on
	 ...
	 */
	@Operation(summary = "Delete a user-defined process", operationId = "deleteCustomProcess", description = "Deletes the data related to this user-defined process, including its process graph.  Does NOT delete jobs or services that reference this user-defined process.", tags = {
			"User-Defined Processes", })
	@ApiResponses(value = {
			@ApiResponse(responseCode = "204", description = "The user-defined process has been successfully deleted"),
	...
	@DeleteMapping(value = "/process_graphs/{process_graph_id}", produces = { "application/json" })
	default ResponseEntity<Void> deleteCustomProcess(
	...
	/**
	 * GET /process_graphs/{process_graph_id} : Full metadata for a user-defined
	 * process Returns all information about a user-defined process, including its
	 * process graph.
	 *
	 * @param processGraphId Unique identifier for a user-defined process.
	 *                       (required)
	 * @return The user-defined process with process graph. (status code 200) or The
	 ...
	 *         in general. * [Common Error Codes](errors.json) (status code 500)
	 */
	@Operation(summary = "Full metadata for a user-defined process", operationId = "describeCustomProcess", description = "Returns all information about a user-defined process, including its process graph.", tags = {
			"User-Defined Processes", "Process Discovery", })
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "The user-defined process with process graph."),
			@ApiResponse(responseCode = "400", description = "The request can't be fulfilled due to an error on client-side, i.e. the request is invalid. The client should not repeat the request without modifications.  The response body SHOULD contain a JSON error object. MUST be any HTTP status code specified in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-6.6). This request MUST respond with HTTP status codes 401 if authorization is required or 403 if the authorization failed or access is forbidden in general to the authenticated user. HTTP status code 404 should be used if the value of a path parameter is invalid.  See also: * [Error Handling](#section/API-Principles/Error-Handling) in the API in general. * [Common Error Codes](errors.json)"),
			@ApiResponse(responseCode = "500", description = "The request can't be fulfilled due to an error at the back-end. The error is never the client’s fault and therefore it is reasonable for the client to retry the exact same request that triggered this response.  The response body SHOULD contain a JSON error object. MUST be any HTTP status code specified in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-6.6).  See also: * [Error Handling](#section/API-Principles/Error-Handling) in the API in general. * [Common Error Codes](errors.json)") })
	@GetMapping(value = "/process_graphs/{process_graph_id}", produces = { "application/json" })
	default ResponseEntity<UserDefinedProcessMeta> describeCustomProcess(
			@Pattern(regexp = "^\\w+$") @Parameter(description = "Unique identifier for a user-defined process.", required = true) @PathVariable("process_graph_id") String processGraphId) {
		return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
	...
	/**
	 * GET /process_graphs : List all user-defined processes This service lists all
	 * user-defined processes (process graphs) of the authenticated user that are
	 * stored at the back-end. It is **strongly RECOMMENDED** to keep the response
	 * size small by omitting larger optional values from the objects in
	 * &#x60;processes&#x60; (e.g. the &#x60;exceptions&#x60;, &#x60;examples&#x60;
	 * and &#x60;links&#x60; properties). To get the full metadata for a secondary
	 * web service clients MUST request &#x60;GET /services/{service_id}&#x60;.
	 *
	 ...
	 * @return JSON array with user-defined processes. (status code 200) or The
	 *         request can&#39;t be fulfilled due to an error on client-side, i.e.
	 *         the request is invalid. The client should not repeat the request
	 *         without modifications. The response body SHOULD contain a JSON error
	 *         object. MUST be any HTTP status code specified in [RFC
	 ...
	 *         in general. * [Common Error Codes](errors.json) (status code 500)
	 */
	@Operation(summary = "List all user-defined processes", operationId = "listCustomProcesses", description = "This service lists all user-defined processes (process graphs) of the authenticated user that are stored at the back-end.  It is **strongly RECOMMENDED** to keep the response size small by omitting larger optional values from the objects in `processes` (e.g. the `exceptions`, `examples` and `links` properties). To get the full metadata for a secondary web service clients MUST request `GET /services/{service_id}`.", tags = {
			"User-Defined Processes", "Process Discovery", })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "JSON array with user-defined processes."),
			@ApiResponse(responseCode = "400", description = "The request can't be fulfilled due to an error on client-side, i.e. the request is invalid. The client should not repeat the request without modifications.  The response body SHOULD contain a JSON error object. MUST be any HTTP status code specified in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-6.6). This request MUST respond with HTTP status codes 401 if authorization is required or 403 if the authorization failed or access is forbidden in general to the authenticated user. HTTP status code 404 should be used if the value of a path parameter is invalid.  See also: * [Error Handling](#section/API-Principles/Error-Handling) in the API in general. * [Common Error Codes](errors.json)"),
			@ApiResponse(responseCode = "500", description = "The request can't be fulfilled due to an error at the back-end. The error is never the client’s fault and therefore it is reasonable for the client to retry the exact same request that triggered this response.  The response body SHOULD contain a JSON error object. MUST be any HTTP status code specified in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-6.6).  See also: * [Error Handling](#section/API-Principles/Error-Handling) in the API in general. * [Common Error Codes](errors.json)") })
	@GetMapping(value = "/process_graphs", produces = { "application/json" })
	default ResponseEntity<UserDefinedProcesses> listCustomProcesses(
		...
		getRequest().ifPresent(request -> {
			for (MediaType mediaType : MediaType.parseMediaTypes(request.getHeader("Accept"))) {
				if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
					String exampleString = "{ \"processes\" : [ null, null ], \"links\" : [ { \"rel\" : \"related\", \"href\" : \"https://example.openeo.org\", \"type\" : \"text/html\", \"title\" : \"openEO\" }, { \"rel\" : \"related\", \"href\" : \"https://example.openeo.org\", \"type\" : \"text/html\", \"title\" : \"openEO\" } ] }";
	...
	/**
	 * PUT /process_graphs/{process_graph_id} : Store a user-defined process Stores
	 * a provided user-defined process with process graph that can be reused in
	 * other processes. If a process with the specified &#x60;process_graph_id&#x60;
	 * exists, the process is fully replaced. The id can&#39;t be changed for stored
	 * user-defined processes. The id must be unique for the authenticated user,
	 * including all pre-defined processes by the back-end. Partially updating
	 * user-defined processes is not supported. To simplify exchanging user-defined
	 * processes, the property &#x60;id&#x60; can be part of the request body. If
	 * the values don&#39;t match, the value for &#x60;id&#x60; gets replaced with
	 * the value from the &#x60;process_graph_id&#x60; parameter in the path.
	 *
	 * @param processGraphId           Unique identifier for a user-defined process.
	 *                                 (required)
	 * @param processGraphWithMetadata Specifies the process graph with its meta
	 *                                 data. (required)
	 * @return The user-defined process has been stored successfully. (status code
	 *         200) or The request can&#39;t be fulfilled due to an error on
	 *         client-side, i.e. the request is invalid. The client should not
	 *         repeat the request without modifications. The response body SHOULD
	 *         contain a JSON error object. MUST be any HTTP status code specified
	 *         in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-6.6). This
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "400", description = "The request can't be fulfilled due to an error on client-side, i.e. the request is invalid. The client should not repeat the request without modifications.  The response body SHOULD contain a JSON error object. MUST be any HTTP status code specified in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-6.6). This request MUST respond with HTTP status codes 401 if authorization is required or 403 if the authorization failed or access is forbidden in general to the authenticated user. HTTP status code 404 should be used if the value of a path parameter is invalid.  See also: * [Error Handling](#section/API-Principles/Error-Handling) in the API in general. * [Common Error Codes](errors.json)"),
			@ApiResponse(responseCode = "500", description = "The request can't be fulfilled due to an error at the back-end. The error is never the client’s fault and therefore it is reasonable for the client to retry the exact same request that triggered this response.  The response body SHOULD contain a JSON error object. MUST be any HTTP status code specified in [RFC 7231](https://tools.ietf.org/html/rfc7231#section-6.6).  See also: * [Error Handling](#section/API-Principles/Error-Handling) in the API in general. * [Common Error Codes](errors.json)") })
	@PutMapping(value = "/process_graphs/{process_graph_id}", produces = { "application/json" }, consumes = {
			"application/json" })
	default ResponseEntity storeCustomProcess(
			@Pattern(regexp = "^\\w+$") @Parameter(description = "Unique identifier for a user-defined process.", required = true) @PathVariable("process_graph_id") String processGraphId,
			@Parameter(description = "Specifies the process graph with its meta data.", required = true) @Valid @RequestBody Process processGraphWithMetadata) {
		return new ResponseEntity<>(HttpStatus.NOT_IMPLEMENTED);
