...
public interface WellKnownApi {
	...
	/**
	 * GET /.well-known/openeo : Supported openEO versions Well-Known URI (see [RFC
	 * 5785](https://tools.ietf.org/html/rfc5785)) for openEO, listing all
	 * implemented openEO versions supported by the service provider. This allows a
	 * client to easily identify the most recent openEO implementation it supports.
	 * By default, a client SHOULD connect to the most recent production-ready
	 * version it supports. If not available, the most recent supported version of
	 * *all* versions SHOULD be connected to. Clients MAY let users choose to
	 * connect to versions that are not production-ready or outdated. The most
	 * recent version is determined by comparing the version numbers according to
	 * rules from [Semantic Versioning](https://semver.org/), especially
	 * [§11](https://semver.org/#spec-item-11). Any pair of API versions in this
	 * list MUST NOT be equal according to Semantic Versioning. The Well-Known URI
	 * is the entry point for clients and users, so make sure it is permanent and
	 * easy to use and remember. Clients MUST NOT require the well-known path
	 * (&#x60;./well-known/openeo&#x60;) in the URL that is specified by a user to
	 * connect to the back-end. A client MUST request
	 * &#x60;https://example.com/.well-known/openeo&#x60; if a user tries to connect
	 * to &#x60;https://example.com&#x60;. If the request to the well-known URI
	 * fails, the client SHOULD try to request the capabilities at &#x60;/&#x60;
	 * from &#x60;https://example.com&#x60;. **This URI MUST NOT be versioned as the
	 * other endpoints.** If your API is available at
	 * &#x60;https://example.com/api/v1.0&#x60;, the Well-Known URI SHOULD be
	 * located at &#x60;https://example.com/.well-known/openeo&#x60; and the URI
	 * users connect to SHOULD be &#x60;https://example.com&#x60;. Clients MAY get
	 * additional information (e.g. title or description) about a back-end from the
	 * most recent version that has the &#x60;production&#x60; flag set to
	 * &#x60;true&#x60;.
	 *
	 * @return List of all available API instances, each with URL and the
	 *         implemented openEO API version. (status code 200) or The request
	...
	default ResponseEntity<WellKnownDiscovery> connect() {
		getRequest().ifPresent(request -> {
			for (MediaType mediaType : MediaType.parseMediaTypes(request.getHeader("Accept"))) {
				if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
					String exampleString = "{ \"versions\" : [ { \"production\" : false, \"api_version\" : \"1.0.1\", \"url\" : \"https://example.com/api/v1.0\" }, { \"production\" : false, \"api_version\" : \"1.0.1\", \"url\" : \"https://example.com/api/v1.0\" } ] }";
