...
public class WCPSReduceFunc {
	...
	String createReduceWCPSString(Vector<Filter> filters, Vector<Aggregate> aggregates, String wcpsEndpoint,
			String openEOEndpoint, JSONObject openEOGraph, String reduceNodeKey, String payLoad, String filterString,
			String collectionVar, String collectionID, String dimension) {
		...
		JSONObject reduceProcesses = processGraph.getJSONObject(reduceNodeKey).getJSONObject("arguments")
				.getJSONObject("reducer").getJSONObject("process_graph");
			...
			for (String reducerField : reducerProcess.keySet()) {
				if (reducerField.equals("result")) {
					...
					if (resultFlag) {
						...
						log.debug("End Reducer Process is : "
								+ reduceProcesses.getJSONObject(endReducerNode).getString("process_id"));
					}
				}
			}
		}
		...
		for (int n = 0; n < reduceNodesArray.length(); n++) {
			for (int a = 0; a < reduceNodesArray.getJSONArray(n).length(); a++) {
				JSONArray fromNodeOfReducers = getReducerFromNodes(reduceNodesArray.getJSONArray(n).getString(a),
						reduceProcesses);
		...
		for (int i = 0; i < reduceNodesSortedArray.length(); i++) {
			reduceProcessesSequence
					.put(reduceProcesses.getJSONObject(reduceNodesSortedArray.getString(i)).getString("process_id"));
		}
		...
		for (int r = 0; r < reduceNodesSortedArray.length(); r++) {
				...
				if (arrayData.get("data") instanceof JSONObject) {
					for (String fromType : arrayData.getJSONObject("data").keySet()) {
						if (fromType.equals("from_parameter")
								&& arrayData.getJSONObject("data").getString("from_parameter").equals("data")) {
							reduceBuilderExtend = createBandWCPSString(collectionID, arrayIndex, reduceNodeKey,
			...
			if (name.equals("count")) {
				...
				if (countArguments.get("data") instanceof JSONObject) {
					for (String fromType : countArguments.getJSONObject("data").keySet()) {
						if (fromType.equals("from_parameter")
								&& countArguments.getJSONObject("data").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("mean")) {
				...
				if (meanArguments.get("data") instanceof JSONObject) {
					for (String fromType : meanArguments.getJSONObject("data").keySet()) {
						if (fromType.equals("from_parameter")
								&& meanArguments.getJSONObject("data").getString("from_parameter").equals("data")) {
							meanPayLoad = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				reduceBuilderExtend = createMeanWCPSString(filters, aggregates, reduceNodeKey, meanPayLoad,
						reduceProcesses, dimension, collectionVar, collectionID);
				reducerPayLoads.put(nodeKey, reduceBuilderExtend);
			...
			if (name.equals("min")) {
				...
				if (minArguments.get("data") instanceof JSONObject) {
					for (String fromType : minArguments.getJSONObject("data").keySet()) {
						if (fromType.equals("from_parameter")
								&& minArguments.getJSONObject("data").getString("from_parameter").equals("data")) {
							minPayLoad = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				reduceBuilderExtend = createMinWCPSString(filters, aggregates, reduceNodeKey, minPayLoad,
						reduceProcesses, dimension, collectionVar, collectionID);
				reducerPayLoads.put(nodeKey, reduceBuilderExtend);
			...
			if (name.equals("max")) {
				...
				if (maxArguments.get("data") instanceof JSONObject) {
					for (String fromType : maxArguments.getJSONObject("data").keySet()) {
						if (fromType.equals("from_parameter")
								&& maxArguments.getJSONObject("data").getString("from_parameter").equals("data")) {
							maxPayLoad = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				reduceBuilderExtend = createMaxWCPSString(filters, aggregates, reduceNodeKey, maxPayLoad,
						reduceProcesses, dimension, collectionVar, collectionID);
				reducerPayLoads.put(nodeKey, reduceBuilderExtend);
			...
			if (name.equals("and")) {
				...
				if (andArguments.get("x") instanceof JSONObject) {
					for (String fromType : andArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& andArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
				...
				if (andArguments.get("y") instanceof JSONObject) {
					for (String fromType : andArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& andArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							andArrayreturn.put(payLoad);
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("or")) {
				...
				if (orArguments.get("x") instanceof JSONObject) {
					for (String fromType : orArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& orArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
				...
				if (orArguments.get("y") instanceof JSONObject) {
					for (String fromType : orArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& orArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							orArrayreturn.put(payLoad);
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("xor")) {
				...
				if (xorArguments.get("x") instanceof JSONObject) {
					for (String fromType : xorArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& xorArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
				...
				if (xorArguments.get("y") instanceof JSONObject) {
					for (String fromType : xorArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& xorArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							xorArrayreturn.put(payLoad);
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("multiply")) {
				...
				if (productArguments.get("x") instanceof JSONObject) {
					for (String fromType : productArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& productArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
				...
				if (productArguments.get("y") instanceof JSONObject) {
					for (String fromType : productArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& productArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							productArrayreturn.put(payLoad);
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("add")) {
				...
				if (sumArguments.get("x") instanceof JSONObject) {
					for (String fromType : sumArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& sumArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
				...
				if (sumArguments.get("y") instanceof JSONObject) {
					for (String fromType : sumArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& sumArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							sumArrayreturn.put(payLoad);
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("subtract")) {
				...
				if (subtractArguments.get("x") instanceof JSONObject) {
					for (String fromType : subtractArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& subtractArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
				...
				if (subtractArguments.get("y") instanceof JSONObject) {
					for (String fromType : subtractArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& subtractArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							subtractArrayreturn.put(payLoad);
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("divide")) {
				...
				if (divideArguments.get("x") instanceof JSONObject) {
					for (String fromType : divideArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& divideArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
				...
				if (divideArguments.get("y") instanceof JSONObject) {
					for (String fromType : divideArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& divideArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							divideArrayreturn.put(payLoad);
						} else if (fromType.equals("from_node")) {
			...
			if (name.contains("linear_scale_range")) {
				String x = null;
				JSONObject linearScaleRangeArguments = reduceProcesses.getJSONObject(nodeKey)
					...
					if ((argType.equals("x")) && linearScaleRangeArguments.get(argType) instanceof JSONObject) {
						for (String fromType : linearScaleRangeArguments.getJSONObject(argType).keySet()) {
							if (fromType.equals("from_parameter") && linearScaleRangeArguments.getJSONObject(argType)
									.getString("from_parameter").equals("data")) {
								x = payLoad;
							} else if (fromType.equals("from_node")) {
			...
			if (name.equals("absolute")) {
				...
				if (absArguments.get("x") instanceof JSONObject) {
					for (String fromType : absArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& absArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("not")) {
				...
				if (notArguments.get("x") instanceof JSONObject) {
					for (String fromType : notArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& notArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				for (String argType : logArguments.keySet()) {
					if ((argType.equals("x")) && logArguments.get(argType) instanceof JSONObject) {
						for (String fromType : logArguments.getJSONObject(argType).keySet()) {
							if (fromType.equals("from_parameter")
									&& logArguments.getJSONObject(argType).getString("from_parameter").equals("data")) {
								x = payLoad;
							} else if (fromType.equals("from_node")) {
				...
				for (String argType : logNArguments.keySet()) {
					if ((argType.equals("x")) && logNArguments.get(argType) instanceof JSONObject) {
						for (String fromType : logNArguments.getJSONObject(argType).keySet()) {
							if (fromType.equals("from_parameter") && logNArguments.getJSONObject(argType)
									.getString("from_parameter").equals("data")) {
								x = payLoad;
							} else if (fromType.equals("from_node")) {
				...
				for (String argType : sqrtArguments.keySet()) {
					if ((argType.equals("x")) && sqrtArguments.get(argType) instanceof JSONObject) {
						for (String fromType : sqrtArguments.getJSONObject(argType).keySet()) {
							if (fromType.equals("from_parameter") && sqrtArguments.getJSONObject(argType)
									.getString("from_parameter").equals("data")) {
								x = payLoad;
							} else if (fromType.equals("from_node")) {
				...
				for (String argType : powArguments.keySet()) {
					if ((argType.equals("base")) && powArguments.get(argType) instanceof JSONObject) {
						for (String fromType : powArguments.getJSONObject(argType).keySet()) {
							if (fromType.equals("from_parameter")
									&& powArguments.getJSONObject(argType).getString("from_parameter").equals("data")) {
								base = payLoad;
							} else if (fromType.equals("from_node")) {
				...
				for (String argType : expArguments.keySet()) {
					if ((argType.equals("p")) && expArguments.get(argType) instanceof JSONObject) {
						for (String fromType : expArguments.getJSONObject(argType).keySet()) {
							if (fromType.equals("from_parameter")
									&& expArguments.getJSONObject(argType).getString("from_parameter").equals("data")) {
								p = payLoad;
							} else if (fromType.equals("from_node")) {
				...
				for (String argType : trigArguments.keySet()) {
					if ((argType.equals("x")) && trigArguments.get(argType) instanceof JSONObject) {
						for (String fromType : trigArguments.getJSONObject(argType).keySet()) {
							if (fromType.equals("from_parameter") && trigArguments.getJSONObject(argType)
									.getString("from_parameter").equals("data")) {
								x = payLoad;
							} else if (fromType.equals("from_node")) {
			...
			if (name.equals("gte")) {
				...
				if (gteArguments.get("x") instanceof JSONObject) {
					for (String fromType : gteArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& gteArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				if (gteArguments.get("y") instanceof JSONObject) {
					for (String fromType : gteArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& gteArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							y = payLoad;
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("gt")) {
				...
				if (gtArguments.get("x") instanceof JSONObject) {
					for (String fromType : gtArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& gtArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				if (gtArguments.get("y") instanceof JSONObject) {
					for (String fromType : gtArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& gtArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							y = payLoad;
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("lte")) {
				...
				if (lteArguments.get("x") instanceof JSONObject) {
					for (String fromType : lteArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& lteArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				if (lteArguments.get("y") instanceof JSONObject) {
					for (String fromType : lteArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& lteArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							y = payLoad;
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("lt")) {
				...
				if (ltArguments.get("x") instanceof JSONObject) {
					for (String fromType : ltArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& ltArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				if (ltArguments.get("y") instanceof JSONObject) {
					for (String fromType : ltArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& ltArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							y = payLoad;
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("neq")) {
				...
				if (neqArguments.get("x") instanceof JSONObject) {
					for (String fromType : neqArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& neqArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				if (neqArguments.get("y") instanceof JSONObject) {
					for (String fromType : neqArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& neqArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							y = payLoad;
						} else if (fromType.equals("from_node")) {
			...
			if (name.equals("eq")) {
				...
				if (eqArguments.get("x") instanceof JSONObject) {
					for (String fromType : eqArguments.getJSONObject("x").keySet()) {
						if (fromType.equals("from_parameter")
								&& eqArguments.getJSONObject("x").getString("from_parameter").equals("data")) {
							x = payLoad;
						} else if (fromType.equals("from_node")) {
				...
				if (eqArguments.get("y") instanceof JSONObject) {
					for (String fromType : eqArguments.getJSONObject("y").keySet()) {
						if (fromType.equals("from_parameter")
								&& eqArguments.getJSONObject("y").getString("from_parameter").equals("data")) {
							y = payLoad;
						} else if (fromType.equals("from_node")) {
	...
	private String createMeanWCPSString(Vector<Filter> filters, Vector<Aggregate> aggregates, String reduceNodeKey,
			String payLoad, JSONObject reduceProcesses, String dimension, String collectionVar, String collectionID) {
		String stretchString = null;
		StringBuilder stretchBuilder = new StringBuilder("");
		...
		for (String tempAxis1 : jsonresp.getJSONObject("cube:dimensions").keySet()) {
			...
			if (tempAxis1UpperCase.contentEquals("DATE") || tempAxis1UpperCase.contentEquals("TIME")
					|| tempAxis1UpperCase.contentEquals("ANSI") || tempAxis1UpperCase.contentEquals("UNIX")) {
		...
		if (dimension.contains("spectral") || dimension.contains("bands")) {
			...
			for (int f = 0; f < filters.size(); f++) {
				...
				if (axis.contains(collectionID)) {
					...
					if (axisUpperCase.equals("DATE") || axisUpperCase.equals("TIME") || axisUpperCase.equals("ANSI")
							|| axisUpperCase.equals("UNIX")) {
			...
			for (int a = 0; a < aggregates.size(); a++) {
				...
				if (aggregates.get(a).getAxis().equals(tempAxis + "_" + collectionID + reduceNodeKey)) {
					stretchBuilder.append(createMeanTempAggWCPSString(filters, aggregates, reduceNodeKey, collectionVar,
							collectionID, aggregates.get(a), payLoad, tempAxis));
...
//						stretchBuilder.append(wcpsAggBuilderMod);
	...
	private String createMaxWCPSString(Vector<Filter> filters, Vector<Aggregate> aggregates, String reduceNodeKey,
			String payLoad, JSONObject reduceProcesses, String dimension, String collectionVar, String collectionID) {
		String stretchString = null;
		StringBuilder stretchBuilder = new StringBuilder("");
		String wcps_endpoint = openEOEndpoint;
		...
		if (dimension.contains("spectral") || dimension.contains("bands")) {
			...
			for (int a = 0; a < aggregates.size(); a++) {
				...
				if (aggregates.get(a).getAxis().equals(tempAxis + "_" + collectionID + reduceNodeKey)) {
					stretchBuilder.append(createTempAggWCPSString(filters, aggregates, reduceNodeKey, collectionVar,
							collectionID, aggregates.get(a), tempAxis));
					log.debug(stretchString);
					String replaceDate = Pattern.compile(tempAxis + "\\(.*?\\)").matcher(payLoad)
							.replaceAll(tempAxis + "\\(\\$pm\\" + reduceNodeKey + ")");
					// String replaceDate = wcpsPayLoad.toString().replaceAll(tempAxis+"\\(.*?\\)",
					// tempAxis+"\\(\\$pm\\)");
					StringBuilder wcpsAggBuilderMod = new StringBuilder("");
	...
	private String createMinWCPSString(Vector<Filter> filters, Vector<Aggregate> aggregates, String reduceNodeKey,
			String payLoad, JSONObject reduceProcesses, String dimension, String collectionVar, String collectionID) {
		String stretchString = null;
		StringBuilder stretchBuilder = new StringBuilder("");
		String wcps_endpoint = openEOEndpoint;
	...
	private String createTempAggWCPSString(Vector<Filter> filters, Vector<Aggregate> aggregates, String reduceNodeKey,
			String collectionVar, String collectionID, Aggregate tempAggregate, String tempAxis) {
	...
	private String createMeanTempAggWCPSString(Vector<Filter> filters, Vector<Aggregate> aggregates,
			String reduceNodeKey, String collectionVar, String collectionID, Aggregate tempAggregate, String payLoad,
			String tempAxis) {
		Filter tempFilter = null;
		for (Filter filter : filters) {
			log.debug("Filter Axis is : ");
		...
		if (tempFilter != null) {
			...
			stringBuilder.append(createFilteredCollectionString(collectionVar, collectionID, tempFilter) + ",");
			stringBuilder.append(tempAxis + ")) using ");
			String meanDateRange1 = Pattern.compile(tempAxis + "\\(.*?\\)").matcher(payLoad)
					.replaceAll(tempAxis + "\\(\\$pm\\" + reduceNodeKey + ")");
			stringBuilder.append(meanDateRange1 + ")/( condense + over $pmm" + reduceNodeKey + " t (imageCrsDomain("
					+ payLoad + ",");
...
//				}
				...
				if ((axisUpperCase.contains("DATE") || axisUpperCase.contains("TIME") || axisUpperCase.contains("ANSI")
						|| axisUpperCase.contains("UNIX")) && !low.contains("$cube")) {
					stringBuilder.append("\"");
				}
				...
				if (high != null && !(high.equals(low))) {
					stringBuilder.append(":");
					if (axisUpperCase.contains("DATE") || axisUpperCase.contains("TIME")
							|| axisUpperCase.contains("ANSI") || axisUpperCase.contains("UNIX")) {
						stringBuilder.append("\"");
					}
	...
	// process under reduce process if recently defined by openEO API for reduce
	// process
